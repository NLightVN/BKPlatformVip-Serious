...
const AdminUsers = () => {
    ...
    const handleBanToggle = async (userId, currentStatus, username) => {
        ...
        try {
            ...
            if (res.code === 1000) {
                ...
                // Nếu là hành động BAN, thực hiện ban luôn các Shop và Sản phẩm của user đó
                if (shouldBan && username) {
                    try {
                        const shopRes = await shopService.getShopByOwner(username);
                        const shops = shopRes.result || [];
                        
                        for (const shop of shops) {
                            // Ban Shop
                            await adminService.banShop(shop.shopId, true);
                            
                            // Lấy sản phẩm của shop và Ban
                            const prodRes = await productService.getProductsByShop(shop.shopId);
                            const products = prodRes.result || [];
                            for (const prod of products) {
                                await adminService.banProduct(prod.productId, true);
                            }
                        }
                        if (shops.length > 0) {
                            toast.info(`Đã tự động khóa ${shops.length} shop và các sản phẩm liên quan.`);
                        }
                    } catch (err) {
                        console.error("Lỗi khi ban shop/sản phẩm liên quan:", err);
                    }
                }
                            ...
                            <tr key={u.userId} style={{ borderBottom: '1px solid #eee' }}>
                                ...
                                <td style={{ padding: '12px' }}>
                                    <button
                                        ...
                                        onClick={() => handleBanToggle(u.userId, u.status, u.username)}
                                        style={{

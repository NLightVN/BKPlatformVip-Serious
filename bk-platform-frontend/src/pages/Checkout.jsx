...
const Checkout = () => {
    ...
    useEffect(() => {
        const calculateTotalShipping = async () => {
            ...
            const toWard = isEditingAddress ? addressForm.wardCode : (tempAddress?.ward?.code || user?.address?.ward?.code);
            const toDistrict = isEditingAddress ? addressForm.districtCode : (tempAddress?.district?.code || user?.address?.district?.code);
    ...
    }, [isEditingAddress, addressForm.wardCode, addressForm.districtCode, user, itemsToCheckout, tempAddress]);
    ...
    const handleSaveAddress = async () => {
        ...
        // Cập nhật local state thay vì gọi API
        const provinceName = provinces.find(p => p.code == addressForm.provinceCode)?.fullName;
        const districtName = districts.find(d => d.code == addressForm.districtCode)?.fullName;
        const wardName = wards.find(w => w.code == addressForm.wardCode)?.fullName;

        const newAddress = {
            name: addressForm.fullname,
            phone: addressForm.phone,
            addressDetail: addressForm.addressDetail,
            province: { code: addressForm.provinceCode, fullName: provinceName },
            district: { code: addressForm.districtCode, fullName: districtName },
            ward: { code: addressForm.wardCode, fullName: wardName }
        };

        setTempAddress(newAddress);
        setIsEditingAddress(false);
        toast.success("Đã cập nhật địa chỉ nhận hàng (Tạm thời)");
    };

    const handleCheckout = async () => {
        const currentAddress = tempAddress || user?.address;
        ...
        try {
            ...
            if (isBuyNow) {
                ...
                res = await orderService.buyNow(item.productId, item.quantity, currentAddress);
            } else {
                // Logic checkout từ cart
                const productIds = itemsToCheckout.map(i => i.productId);
                res = await orderService.checkout(productIds, currentAddress);
            }
    ...
    const renderAddressString = () => {
        const addr = tempAddress || user?.address;
        if (!addr) return <span style={{ color: 'red' }}>Chưa có địa chỉ</span>;
        const detail = addr.addressDetail || '';
        const ward = addr.ward?.fullName || '';
        const district = addr.district?.fullName || '';
        const province = addr.province?.fullName || '';
            ...
            <main className="main-content" style={{ display: 'block', padding: '40px' }}>
                ...
                <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap', maxWidth: '1200px', margin: '0 auto' }}>
                        ...
                        <div style={{ background: 'white', padding: '25px', borderRadius: '10px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                            ...
                            {!isEditingAddress ? (
                                <div style={{ lineHeight: '1.8', fontSize: '15px' }}>
                                    <p><strong>Người nhận:</strong> {tempAddress?.name || user.fullname}</p>
                                    <p><strong>SĐT:</strong> {tempAddress?.phone || user.address?.phone}</p>
                                ...
                                <div className="address-form">
                                    ...
                                    <div style={{ display: 'flex', gap: '10px' }}>
                                        <button className="btn btn-primary" onClick={handleSaveAddress}>Lưu địa chỉ</button>
                    ...
                    <div style={{ flex: 1, background: 'white', padding: '25px', borderRadius: '10px', height: 'fit-content', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                        ...
                        <button className="btn btn-primary" style={{ width: '100%', marginTop: '20px', padding: '12px', fontSize: '16px' }} onClick={handleCheckout} disabled={loading}>

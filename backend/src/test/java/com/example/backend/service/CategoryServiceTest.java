...
class CategoryServiceTest {
    ...
    void deleteCategory_usedByProduct() {
        try (MockedStatic<SecurityUtil> mockedSecurityUtil = mockStatic(SecurityUtil.class)) {
            ...
            // Create a real product with a mocked mutable set of categories
            Product mockProduct = mock(Product.class);
            Set<Category> productCategories = mock(Set.class);
            when(mockProduct.getCategories()).thenReturn(productCategories);

            // Create category with the product
            Category categoryWithProducts = Category.builder()
                    .categoryId("cat-1")
                    .name("electronics")
                    .products(new java.util.HashSet<>(Set.of(mockProduct))) // Use mutable HashSet
                    .build();

            when(categoryRepository.findByName("electronics")).thenReturn(Optional.of(categoryWithProducts));
            doNothing().when(categoryRepository).delete(categoryWithProducts);

            // Act - service should remove category from products and then delete it
            categoryService.deleteCategory("Electronics");

            // Assert - verify category was removed from each product's categories
            verify(productCategories).remove(categoryWithProducts);
